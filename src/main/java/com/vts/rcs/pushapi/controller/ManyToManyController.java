...
public class ManyToManyController {
	...
	@RequestMapping(value = "/apiservice/manyToManyMessage", method = { RequestMethod.OPTIONS,
			RequestMethod.POST }, produces = { "application/json" })
	public String sendTextMsg(@RequestBody List<JsonRequest> jsonRequestList, HttpServletResponse resp,
			HttpServletRequest req) throws JsonProcessingException {

		logger.info("Received request to /apiservice/sendmessage (Batch Mode)");
		...
		if (jsonRequestList == null || jsonRequestList.isEmpty()) {
			...
			response.setDesc("No requests found in payload.");
			responseList.add(response);
			return objectMapper.writeValueAsString(responseList);
		}

		for (JsonRequest jsonRequest : jsonRequestList) {
			...
			if (isNullOrEmpty(jsonRequest.getUserName()) || isNullOrEmpty(jsonRequest.getPhoneNumber())
				...
				logger.warn("Mandatory fields missing for one request: username, phoneNumber, or templateCode");
				PushResponse response = new PushResponse();
				response.setCode("-100");
			...
			if (user == null) {
				...
				response.setDesc("Invalid user: " + jsonRequest.getUserName());
				responseList.add(response);
				continue;
			}

//			// Validate token once per user
//			String token = req.getHeader("authorization");
//			if (token == null || !token.equals(user.getApiKey())) {
//				logger.warn("Unauthorized access for user: {}", user.getUserName());
//				resp.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
//				PushResponse response = new PushResponse();
//				response.setCode("-102");
//				response.setDesc("Unauthorized: Invalid API Key for user " + user.getUserName());
//				responseList.add(response);
//				continue;
//			}
			...
			for (String rawNumber : phoneNumbers) {
				...
				try {
					logger.info("Processing number: {} for user: {}", number, jsonRequest.getUserName());
		...
		logger.info("Completed processing all batch requests.");
		return objectMapper.writeValueAsString(responseList);
	}
